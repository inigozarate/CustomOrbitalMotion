...
namespace RBD
{
namespace mrigidBodySolvers
{
    defineTypeNameAndDebug(mNewmark, 0);
    addToRunTimeSelectionTable(mrigidBodySolver, mNewmark, dictionary);
}
}
}
...
Foam::RBD::mrigidBodySolvers::mNewmark::mNewmark
(
    mrigidBodyMotion& body,
    const dictionary& dict
)
:
    mrigidBodySolver(body),
...
void Foam::RBD::mrigidBodySolvers::mNewmark::solve
(
    const scalarField& tau,
    const Field<spatialVector>& fx
)
{
    // Accumulate the mrestraint forces
    scalarField rtau(tau);
    Field<spatialVector> rfx(fx);
    model_.applyRestraints(rtau, rfx, state());
    Info << " Applying restraint to: " << endl;
