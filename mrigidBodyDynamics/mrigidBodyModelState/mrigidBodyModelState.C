...
Foam::RBD::mrigidBodyModelState::mrigidBodyModelState
(
...
scalar amplitude = motionDict.lookupOrDefault<scalar>("amplitude", 0);
scalar omega     = motionDict.lookupOrDefault<scalar>("omega", 0);

// Overwrite index 3
if (qDdot_.size() > 3)
{
    qDot_[2] =  amplitude*omega;
}
else
{
    FatalErrorInFunction
        << "qDdot_ has fewer than 4 DoFs; cannot assign qDdot_[3]"
        << exit(FatalError);
}
    ...
    Info<<"Initializing with zero velocity"<<endl;
}
